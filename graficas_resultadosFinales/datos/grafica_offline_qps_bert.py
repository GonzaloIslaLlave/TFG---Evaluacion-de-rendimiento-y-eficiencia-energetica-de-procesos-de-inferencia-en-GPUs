...
#plt.title('Batch vs. Resultado QPS en Bert')

idx = (np.abs(np.array(X1) - 70)).argmin()
y_value = Y1[idx]
plt.plot([70, 70], [0, y_value], linestyle='--', color='r')

idx = (np.abs(np.array(X2) - 30)).argmin()
y_value = Y2[idx]
plt.plot([30, 30], [0, y_value], linestyle='--', color='r')

idx = (np.abs(np.array(X3) - 20)).argmin()
y_value = Y3[idx]
plt.plot([20, 20], [0, y_value], linestyle='--', color='r')
